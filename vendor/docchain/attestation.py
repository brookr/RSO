    ...
    *,
    ...
    if interactive:
        ...
        with cast_wallet_args_from_env(
            ...
            result = run(command, check=True, capture_output=True, text=True)
    receipt = receipt_from_cast_output(result.stdout)
...
def receipt_from_cast_output(output: str) -> dict[str, object]:
    """Extract the receipt object from `cast send --json` output.

    cast versions differ in what they print around the receipt (warnings,
    progress lines), so scan from the end for the first parseable JSON object
    instead of assuming the receipt is exactly the last line. Learned in RSO
    sweeper production.
    """
